...
export class DeliverySlotsController {
    ...
    @Get()
    get(@Query('type') type: string, @Query('startDate') startDate: string, @Query('endDate') endDate: string) {
        const data = this.deliverySlotsService.getByTypeAndBetweenDate(type, startDate, endDate);
